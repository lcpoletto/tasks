...
resource "aws_api_gateway_method_response" "notes_put_500_response" {
	rest_api_id = "${aws_api_gateway_rest_api.tasks_api.id}"
    resource_id = "${aws_api_gateway_resource.note_detail_resource.id}"
    http_method = "${aws_api_gateway_method.notes_put_method.http_method}"
    status_code = 500
...
resource "aws_api_gateway_integration_response" "notes_put_500_integration_response" {
    rest_api_id = "${aws_api_gateway_rest_api.tasks_api.id}"
    resource_id = "${aws_api_gateway_resource.note_detail_resource.id}"
    http_method = "${aws_api_gateway_method.notes_put_method.http_method}"
    status_code = "${aws_api_gateway_method_response.notes_put_500_response.status_code}"
    selection_pattern = "(\n|.)+"
